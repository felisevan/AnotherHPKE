...
class Ciphersuite:
    def __init__(self, kem_id: KemIds, kdf_id: KdfIds, aead_id: AeadIds):
        self.id = concat(b"HPKE", I2OSP(kem_id, 2), I2OSP(kdf_id, 2), I2OSP(aead_id, 2))
    ...
    def SetupBaseS(
        ...
        return enc, ContextFactory(self, RoleIds.SENDER).key_schedule(
            ModeIds.MODE_BASE,
            shared_secret,
            info,
            self._default_psk,
            self._default_psk_id,
        )

    def SetupBaseR(self, enc: bytes, skR: PrivateKeyTypes, info: bytes | None = None):
        shared_secret = self.kem.decap(enc, skR)
        return ContextFactory(self, RoleIds.RECIPIENT).key_schedule(
            ModeIds.MODE_BASE,
            shared_secret,
            info,
            self._default_psk,
    ...
    def SetupPSKS(
        ...
        shared_secret, enc = self.kem.encap(pkR, skE, pkE)
        return enc, ContextFactory(self, RoleIds.SENDER).key_schedule(
            ModeIds.MODE_PSK, shared_secret, info, psk, psk_id
        )
    ...
    def SetupAuthPSKS(
        ...
        shared_secret, enc = self.kem.auth_encap(pkR, skS, skE, pkE)
        return enc, ContextFactory(self, RoleIds.SENDER).key_schedule(
            ModeIds.MODE_AUTH_PSK, shared_secret, info, psk, psk_id
        )
